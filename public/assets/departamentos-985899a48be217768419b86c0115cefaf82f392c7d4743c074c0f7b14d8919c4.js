...
(function() {
  $(document).ready(function() {
    ...
    $("#empleado_supervisor").change(function() {
      var val;
      val = $(this).val();
      return $("#empleado_multiple option").each(function(index, value) {
        if ($(value).val() === val) {
          return $(value).attr("selected", true);
        } else {
          return $(value).attr("selected", false);
        }
      });
    });
    ...
    return $('#fuelux-wizard-container').on('finished.fu.wizard', function(evt, data) {
      return $("form").submit();
      ...
      if (data.step === 2 && data.direction === 'next') {
        if ($("select[name='departamento[empleado_ids][]_helper2'] option").length === 0) {
          ...
          $("#supervisor_id").empty();
          $("select[name='departamento[empleado_ids][]_helper2'] option").each(function(index, value) {
            $("#supervisor_id").append($(value).clone());
            if ($("#dpto_supervisor_id").length) {
              return $("#supervisor_id").val($("#dpto_supervisor_id").val());
            }
          });
        }
      }
      if (data.step === 3 && data.direction === 'next') {
        if ($("#supervisor_id").val() === null) {
          evt.preventDefault();
          return alert("No ha seleccionado ningun supervisor.");
        } else {
          ...
          $("#ulEmpleados").empty();
          return $("select[name='departamento[empleado_ids][]_helper2'] option").each(function(index, value) {
            if ($(value).val() !== $("#supervisor_id").val()) {
              return $("#ulEmpleados").append($("<li>").append($(value).clone().text()));
            }
          });
        }
      }
